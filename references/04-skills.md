...
> **모드 주의.** 아래 skill-creator 기반 **스킬 디렉토리 생성은 대화형(Claude Code) 모드** 기준이다. **서비스(ProcessGPT deepagent) 모드**에서는 로컬 `.bpmn/` 파일을 쓰지 않고 최종 JSON 출력 계약의 `skills`(스킬명 배열)·`activity.skills` 로만 반영한다 — [09-service-execution.md](09-service-execution.md) 참조. 두 모드 공통으로 **단계 A(의도 정리)와 단계 C(JSON 반영)** 는 동일하게 수행하고, 단계 B(파일 생성)만 대화형 모드에서 실행한다.
